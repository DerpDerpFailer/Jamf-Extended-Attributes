...
# Récupération de la date d'expiration du certificat
input=$(security find-certificate -c $userEMail -p | openssl x509 -text | grep "Not After")
...
month=$(echo "$input" | awk '{print $4}')
day=$(echo "$input" | awk '{print $5}')
time=$(echo "$input" | awk '{print $6}')
year=$(echo "$input" | awk '{print $7}')
